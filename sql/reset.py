...
TEMPLATES_DIR = SQL_DIR / "templates"
SESSIONS_DIR  = SQL_DIR / "sessions"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Reset SQL practice sessions.")
    parser.add_argument(
        "targets",
        nargs="*",
    ...
    print(f"\nCreated session: sql/sessions/{today}/")
    for name in created:
        print(f"  ✓ {name}.ipynb")

    print(f"\nLaunch with:")
    print(f"  uv run jupyter lab sql/sessions/{today}/")
